        ...
        if(session != null){
        	...
        	if(isLogon==true){ 
    			...
    			out.print("<br><br>"
    					...
    					+ "	<a href='/pro09/member/memberLogin.html'><button>로그아웃</button></a></h2>");
        		out.print("</body></html>");
    		
    		} else{//(isLogon==false)로그인 상태가 아니면 로그인 창으로 이동합니다.
    		
   			//기존 세션 무효화
    		session.invalidate();
    		//로그인 페이지로 리다이렉트
    		response.sendRedirect("/pro09/member/memberLogin.html");
    		}
