...
public class MemberDAO {
	...
	public List<MemberVO> listMembers() {
		...
		try {
			...
			while (rs.next()) {
				...
				MemberVO memberVO = new MemberVO();	//각 컬럼값을 다시 MemberVO 객체의
												//속성에 설정합니다.
				memberVO.setId(id);					    
	...
	public MemberVO memberInfo(String id) {
		...
		try {
			...
			String idSearchedMemberSql =
					"SELECT * FROM hr.t_member where id = ?";
				
			pstmt = con.prepareStatement(idSearchedMemberSql);
		...
		return memberVO;	//조회한 레코드의 개수만큼 MemberVO 객체를 저장한 
	}					//ArrayList를 반환합니다.
	...
	public List<MemberVO> listMembers(MemberVO memberVO) {
		...
		try {
			...
			if(_name != null && _name.length() != 0) {
				...
				searchedMemberSql += " WHERE UPPER(name) LIKE ?"; //문자열의 맨 앞에 빈칸이 포함됩니다.
	...
	public List<MemberVO> listMembersJoinDateInterval(String startDay, String endDay) {
		...
		List<MemberVO> memeberList = new ArrayList<MemberVO>();
		...
		try {
			...
			while (rs.next()) {
				...
				memeberList.add(memberVO);
				
				//memberList 객체에 저장된 VO를 정렬시킵니다.
				Collections.sort(memeberList);
			}
			rs.close();
			pstmt.close();
			con.close();
		...
		return memeberList;	//조회한 레코드의 개수만큼 MemberVO 객체를
	...
	public void addMember(MemberVO memberVO) {
		...
		try {
			...
			String insertMemberStr = "INSERT INTO hr.t_member values(?,?,?,?,DEFAULT)";
	...
	public void delMember(String id) {
		...
		try {
			...
			String deleteMemberStr = "DELETE FROM hr.t_member WHERE id=?";
